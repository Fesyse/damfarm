...
// Memoized resource card component to prevent unnecessary re-renders
const ResourceCard = memo(
  ({ plant, count }: { plant: string; count: number }) => {
    const emoji = useMemo(() => {
      return {
        carrot: "🥕",
        potato: "🥔",
...
// Memoized interaction prompt component
const InteractionPrompt = memo(
  ({ interaction }: { interaction: InteractionPoint }) => {
    return (
      <motion.div
        key={"interaction"}
...
export function FarmGame() {
  const gameStore = useGameStore((state: GameState) => state)
  ...
  // Memoize the game resources
  const resourceEntries = useMemo(() => {
    return Object.entries(gameStore.resources)
  }, [gameStore.resources])

  // Memoize dialog change handler
  ...
  return (
    <div className='w-full h-screen relative'>
      ...
      <AnimatePresence mode='wait'>
        {nearInteraction && <InteractionPrompt interaction={nearInteraction} />}
      </AnimatePresence>
          ...
          >
            <div className='bg-white/80 backdrop-blur-md rounded-lg shadow-lg p-4 max-w-6xl mx-auto'>
              ...
              <div className='grid grid-cols-2 md:grid-cols-6 gap-4'>
                {resourceEntries.map(([plant, count], i) => (
                  <ResourceCard key={i} plant={plant} count={count as number} />
                ))}
              </div>
            </div>
          </motion.div>
